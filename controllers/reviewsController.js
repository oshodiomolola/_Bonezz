...
const updateReviews = async (req, res, next) => {
  try {
    ...
    if (review.user.id === req.user.id) {
      ...
      if (!updateReviews)
        return next(new appError("no reviews for update", 404));
      res.status(200).json({
        result: "SUCCESSFUL",
...
const deleteReviews = async (req, res, next) => {
  try {
    const review = await Reviews.findOne({ blog: req.params.blogId });
    if (review.user.id === req.user.id) {
      const deleteReviews = await Reviews.findByIdAndDelete(
        req.params.reviewId
      );
      if (!deleteReviews)
        return next(new appError("No review has been deleted", 404));
      res.status(203).json({
        result: "SUCCESSFUL",
